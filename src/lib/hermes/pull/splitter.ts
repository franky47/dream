...
// The projection orders each session's header row ahead of its messages. The
// header carries `latestMessageTime`, which routes the whole snapshot to the
// UTC-day bucket of its most recent message.
    ...
    for await (const line of opts.lines) {
      ...
      if (env.sessionId !== currentSessionId) {
        ...
        const header = parseLine(
          sessionHeaderSchema,
          line,
          `hermes/${opts.machine}: session ${env.sessionId} header`,
        )
        const dir = path.join(
          opts.dataDir,
      ...
      else metrics.messages_pulled += 1
    }
  } finally {
    if (handle !== null) await handle.close()
  }
